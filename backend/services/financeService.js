...
const hitungSkorOBS = ({
 ...
 const fhs = Math.round((total / max) * 100);


 return {
   total: fhs,
   indicatorScore: { total, max },
   surveyTersedia: s1 !== null,
   dimensi: {
     objective:  { savingRatio: o1, expenseRatio: o2, cashFlow: o3, debtRatio: o4 },
     behavioral: { consumptiveRatio: b1, transactionFrequency: b2, impulsiveSpending: b3, budgetDiscipline: b4 },
...
const makeInsightRecords = (insight) => {
 return insight.map((message, index) => ({
   insight_type: message.includes("Subjektif") ? "subjective" : index <= 2 ? "objective" : "behavioral",
   title: message.split(".")[0].slice(0, 150),
   message,
   severity: message.includes("sangat baik") || message.includes("Bagus") ? "success" : "info",
 }));
};


const makeWarningRecords = ({ warning, savingRatio, expenseRatio, consumptiveRatio, debtRatio, expenseCount }) => {
 return warning.map((message, index) => {
   let code = `GENERAL_WARNING_${index + 1}`;
   ...
   if (message.toLowerCase().includes("cash flow")) {
     ...
     triggerValue = expenseRatio;
     thresholdValue = 1;
   } else if (message.toLowerCase().includes("saving ratio")) {
     code = "LOW_SAVING_RATIO";
     triggerValue = savingRatio;
     thresholdValue = 0.2;
   } else if (message.toLowerCase().includes("pengeluaran")) {
     code = "HIGH_EXPENSE_RATIO";
     triggerValue = expenseRatio;
     thresholdValue = 0.9;
   } else if (message.toLowerCase().includes("cicilan") || message.toLowerCase().includes("hutang")) {
     code = "HIGH_DEBT_RATIO";
     triggerValue = debtRatio;
     thresholdValue = 0.2;
   } else if (message.toLowerCase().includes("konsumtif")) {
     code = "HIGH_CONSUMPTIVE_RATIO";
     triggerValue = consumptiveRatio;
     thresholdValue = 0.35;
   } else if (message.toLowerCase().includes("frekuensi")) {
     code = "HIGH_TRANSACTION_FREQUENCY";
     triggerValue = expenseCount;
     thresholdValue = 35;
   }
...
const makeRecommendationRecords = ({ savingRatio, expenseRatio, consumptiveRatio, debtRatio, income, expense, kategori }) => {
 const records = [];
 ...
 if (income === 0) {
   records.push({
     ...
     title: "Catat pendapatan utama",
     message: "Masukkan transaksi pendapatan terlebih dahulu agar sistem dapat menghitung rasio dan skor kesehatan finansial dengan akurat.",
     priority: "high",
   });
   return records;
 }
 ...
 if (savingRatio < 0.1) {
   records.push({
     ...
     title: "Naikkan saving ratio bertahap",
     message: "Mulai sisihkan minimal 10% dari pendapatan di awal bulan sebelum melakukan pengeluaran konsumtif.",
     priority: "high",
   });
 } else if (savingRatio < 0.2) {
   records.push({
     recommendation_type: "saving",
     title: "Dorong saving ratio ke 20%",
     message: "Saving ratio sudah positif, tetapi masih perlu ditingkatkan. Targetkan 20% terlebih dahulu sebelum mengejar 30%.",
     priority: "medium",
   });
 }
 ...
 if (expenseRatio > 0.9) {
   records.push({
     ...
     title: "Turunkan pengeluaran tetap dan variabel",
     message: "Pengeluaran hampir menyamai atau melebihi pendapatan. Evaluasi tiga kategori pengeluaran terbesar bulan ini dan tetapkan batas belanja mingguan.",
     priority: "high",
   });
 }


 if (consumptiveRatio >= 0.35) {
   const topConsumptive = kategori.konsumtif?.[0]?.category || "kategori konsumtif terbesar";
   records.push({
     recommendation_type: "consumptive_control",
     title: "Batasi pengeluaran konsumtif terbesar",
     message: `Pengeluaran konsumtif cukup dominan. Mulai batasi kategori "${topConsumptive}" dan gunakan sistem limit mingguan.`,
     priority: consumptiveRatio >= 0.5 ? "high" : "medium",
   });
 }
 ...
 if (debtRatio >= 0.2) {
   records.push({
     recommendation_type: "debt_control",
     title: "Jaga cicilan agar tidak bertambah",
     message: "Rasio cicilan/hutang mulai signifikan. Prioritaskan pembayaran kewajiban berbunga tinggi dan hindari mengambil cicilan baru.",
     priority: debtRatio >= 0.4 ? "high" : "medium",
   });
 }


 if (records.length === 0 && expense > 0) {
   records.push({
     recommendation_type: "budgeting",
     title: "Pertahankan pola anggaran",
     message: "Rasio utama masih cukup terkendali. Pertahankan pencatatan rutin dan evaluasi kategori pengeluaran terbesar setiap akhir bulan.",
     priority: "low",
   });
 }
...
export const calculateSummary = (transactions, survey = null) => {
 ...
 const debtTotal = transactions
   .filter((t) => t.type === "expense" && t.kelompok_analisis === "kewajiban")
 ...
 const { total: skorKesehatan, indicatorScore, surveyTersedia, dimensi } = noData
   ? {
       total: 0,
       indicatorScore: { total: 0, max: 0 },
       surveyTersedia: false,
       dimensi: {
         objective:  { savingRatio: 0, expenseRatio: 0, cashFlow: 0, debtRatio: 0 },
         behavioral: { consumptiveRatio: 0, transactionFrequency: 0, impulsiveSpending: 0, budgetDiscipline: 0 },
 ...
 return {
   ...
   rawRatio: {
     pengeluaran: expenseRatio,
     tabungan: savingRatio,
     konsumtif: consumptiveRatio,
     hutang: debtRatio,
     frekuensiTransaksi: expenseCount,
   },


   transactionCount: transactions.length,
   consumptiveTransactionCount: transactions.filter((t) => t.type === "expense" && t.kelompok_analisis === "konsumtif").length,


   kategori: {
     ...kategori,
     totalKonsumtif: konsumtifTotal,
     totalHutang: debtTotal,
     totalProduktif: kategori.produktif.reduce((s, k) => s + k.total, 0),
   },
   insight,
   warning,
   recommendation: makeRecommendationRecords({ savingRatio, expenseRatio, consumptiveRatio, debtRatio, income, expense, kategori }).map((r) => r.message),
   insightRecords: makeInsightRecords(insight),
   warningRecords: makeWarningRecords({ warning, savingRatio, expenseRatio, consumptiveRatio, debtRatio, expenseCount }),
   recommendationRecords: makeRecommendationRecords({ savingRatio, expenseRatio, consumptiveRatio, debtRatio, income, expense, kategori }),
 };
};
